> 已归档（2026-09-14）。原路径：`.doc/TODO_BC_PPO_DAPG.md`。本文保留当时方案、参数与实验记录，当前操作请见[文档索引](../README.md)和[训练指南](../TODO_BC_PPO_DAPG.md)。
...
| 成功口径 | 相对闭爪前抬升 0.15 m，收回后连续保持 0.5 s | 奖励要求目标质心高度 ≥0.12 m、距抓取点 <0.12 m、线速度 <1.2 m/s，同时连续保持 0.1 s；举升进度奖励也有距离和速度门控 |
| 掉落终止 | 使用当前环境终止条件 | 曾达到 0.12 m 后跌到 0.05 m 以下即终止，重置时清除历史 |
